...
describe('Max app functional testing', function() {
  ...
  it('Customer care must be able to fix a new appointment for the patient', function(done) {
    ...
    .elementById("table record", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementByTagName('html', function(err, el) {
      b.next('text', el, function(err, text) {
        expect("" + text).to.contain("" + "Fix appointment");
      });
    })
    .elementById("ui field", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementByCssSelector("div.selected.item", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementById("ui orange button", function(err, el) {
      b.next('clickElement', el, noop);
