...
var noop = function() {},
    ...
    describe('Max app functional testing', function() {
      ...
      it('Patient must be able to view phlebos location', function(done) {
    ...
    .elementById("//div[@class='rt-tbody']//div[.='Assigned']", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementByCssSelector("i.globe.icon", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementByTagName('html', function(err, el) {
      b.next('text', el, function(err, text) {
        expect("" + text).to.contain("" + "Location of the phlebotomist");
      });
    })
    .close(function(err) {
      done(err);
