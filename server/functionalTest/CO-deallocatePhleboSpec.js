...
describe('Max app functional testing', function() {
  ...
  it('Coordinator care must be able to deallocate a phlebo for an appointment', function(done) {
    ...
    .elementById("//div[@id='app']//td[.='Sunil Kumar']", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementById("//div[@class='center']//button[.='Deallocate']", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementByTagName('html', function(err, el) {
      b.next('text', el, function(err, text) {
        expect("" + text).to.contain("" + "Are you sure you want to deallocate the phlebotomist?");
      });
    })
    .elementByTagName('html', function(err, el) {
      b.next('text', el, function(err, text) {
    ...
    .elementById("//div[6]/div/div/div/div[2]/div/div", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementByTagName('html', function(err, el) {
      b.next('text', el, function(err, text) {
        expect("" + text).to.contain("" + "Deallocated successfully");
      });
    })
    .close(function(err) {
      done(err);
