...
var noop = function() {},
    ...
    describe('Max app functional testing', function() {
      ...
      it('Patient must be able to cancel his/her appointment', function(done) {
    ...
    .elementById("//div[@class='rt-tbody']//div[.='Unassigned']", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementByCssSelector("i.remove.icon", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementById("//div[6]/div/div/div/div/div/button[2]", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .close(function(err) {
      done(err);
