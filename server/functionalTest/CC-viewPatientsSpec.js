...
describe('Max app functional testing', function() {
  ...
  it('Customer care must be view  all the details of a patients', function(done) {
    ...
    .elementByTagName('html', function(err, el) {
      b.next('text', el, function(err, text) {
        expect("" + text).to.contain("" + "PA9");
      });
    })
    .elementById("table row", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementByTagName('html', function(err, el) {
      b.next('text', el, function(err, text) {
        expect("" + text).to.contain("" + "Ramesh IyerFix appointmentIDPA91Age221Gendermale1Contact98745462141DOB94//1/12/11AddressKrishnarajpu");
      });
    })
    .close(function(err) {
      done(err);
