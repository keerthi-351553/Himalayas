...
var noop = function() {},
    ...
    describe('Max app functional testing', function() {
      ...
      it('Patient must be able to modify his/her profile', function(done) {
    ...
    .elementByCssSelector("textarea", function(err, el) {
      b.next('clear', el, function(err) {
        b.next('type', el, "Hosa road,Bangalore", noop);
      });
    })
    .elementById("//div[@id='app']//button[.='save changes']", function(err, el) {
      b.next('clickElement', el, noop);
    })
    .elementByTagName('html', function(err, el) {
    b.next('text', el, function(err, text) {
      expect("" + text).to.contain("" + "Updated Sucessfully");
    });
  })
    .close(function(err) {
      done(err);
